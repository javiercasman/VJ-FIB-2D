...
void Item::init(string type, const glm::ivec2& tileMapPos, ShaderProgram& shaderProgram)
{
	...
	if (type == "Key") {
		...
		sprite = Sprite::createSprite(tileSize, glm::vec2(5.f/18.f, 1), &spritesheet, &shaderProgram);//son 5 de anchura
		...
		tileSize = glm::ivec2(12,24);
		sprite = Sprite::createSprite(tileSize, glm::vec2(5.f / 18.f, 1), &spritesheet, &shaderProgram);
		sprite->setNumberAnimations(1);
		sprite->setAnimationSpeed(IDLE, 6);
		sprite->addKeyframe(IDLE, glm::vec2(1 * 5.f/18.f, 0.f));
		sprite->changeAnimation(0);
		sprite->setPosition(glm::vec2(float(tileMapDispl.x + posItem.x), float(tileMapDispl.y + posItem.y)));
	}
	else if (type == "Stopwatch") {
		tileSize = glm::ivec2(16, 32);
		sprite = Sprite::createSprite(tileSize, glm::vec2(8.f / 18.f, 1), &spritesheet, &shaderProgram);
		sprite->setNumberAnimations(1);
		sprite->setAnimationSpeed(IDLE, 6);
		sprite->addKeyframe(IDLE, glm::vec2(2 * 5.f / 18.f, 0.f));
		sprite->changeAnimation(0);
		sprite->setPosition(glm::vec2(float(tileMapDispl.x + posItem.x), float(tileMapDispl.y + posItem.y)));
	}
	//ya haremos el otro objeto
}
